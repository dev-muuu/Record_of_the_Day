...
<aside>
	...
	→ 데이터 binding을 onBindViewHolder 메서드에서 구현하는 것이 아닌, 각 viewHolder 클래스에 setItem()메서드를 구현해 아이템을 넘겨주고, 
	각 클래스에서 binding되도록 코드 구현

	> 사실 데이터 binding에 대해 기존에 계속 해당 메서드내에서 처리하는 방식으로 구현을 했었다. 

	하지만 각 viewHolder클래스에서 구현함에 따라 ui요소 접근도 더 간단하게 할 수 있고, onBindViewHolder 메서드 내의 코드 길이도 확실히 줄일 수 있기 때문에 
	더 좋은 방식이라 생각되어 예제 코드를 반영하게 되었다.
