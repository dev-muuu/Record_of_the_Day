...
오늘은 채팅방에 갤러리와 카메라를 연결시키고, 이후 채팅방 메시지로 보내는 것까지 구현을 하였다. 

갤러리, 카메라 권한 제어 및 결과값 반환은 이전에 재치 등록에서 미리 코드를 짜놨었기 때문에, 기존 코드를 참고하면서 복습하는 정도로 진행됐다. 

이전에 텍스트 메시지가 전송되었을 때, 화면에 출력되는 것까지는 구현을 해놓은 상태였는데, 이미지 메시지를 전송하려고 하니까 텍스트하고는 다른 방식으로 진행이 필요할 것 같아, 새로운 레이아웃 아이템과 함수 등을 생성해 기능 구현을 진행했다. 

채팅 서비스에서

1. 수신사/발신자, 이미지/텍스트에 따른 화면 출력을 어떻게 다르게 하는지
2. 1의 내용을 어떤 구조로 짜고, 코드로 구현해야 하는지 
3. 채팅 데이터 클래스 구현 

해당 부분들에 대해 정리가 필요할 것 같아서 이 부분을 복기하며 정리해보고자 한다. 

---

### #1 수신사/발신자, 이미지/텍스트에 따른 화면 출력을 어떻게 다르게 하는지

→ onCreateViewHolder내 viewType 매개변수 사용

사실 2와 연결되는 부분이다. 1과  2를 같이 고려해야만 한다. 
...
`onCreateViewHolder` 에서 `viewType`은  `getItemViewType` 메서드를 통해 전달되는 것이다. 
...
`getItemViewType`의 메서드의 매개변수를 보면, 전달 받은 데이터의 index를 뜻하는 `position`을 넘겨준다. 

즉, adapter에 전달하는 데이터가 viewType에 대한 정보도 포함하고 있으면 된다는 뜻이다. 
...
viewType에 대한 정보는 추가로 클래스를 구현해, 필요한 type만큼 원하는 정수값으로 할당해주면 된다. 

---

### #2 1의 내용을 어떤 구조로 짜고, 코드로 구현해야 하는지

→ RecyclerView Adapter 내 각각 다른 ViewHolder 생성

`onCreateViewHolder` 에서 상황에 맞는 ViewHolder 클래스를 호출해 생성하면, 이후 데이터 binding을 진행하게 된다. 
...
`onBindViewHolder` 메서드에서 `viewHolder` 매개변수를 사용해 어느 클래스에서 넘어온 것인지를 확인한다. 

<aside>
❗ 각 viewHolder 클래스의 setItem() 메서드 주목

→ 데이터 binding을 onBindViewHolder 메서드에서 구현하는 것이 아닌, 각 viewHolder 클래스에 setItem()메서드를 구현해 아이템을 넘겨주고, 각 클래스에서 binding되도록 코드 구현

> 사실 데이터 binding에 대해 기존에 계속 해당 메서드내에서 처리하는 방식으로 구현을 했었다. 
하지만 각 viewHolder클래스에서 구현함에 따라 ui요소 접근도 더 간단하게 할 수 있고, onBindViewHolder 메서드 내의 코드 길이도 확실히 줄일 수 있기 때문에 더 좋은 방식이라 생각되어 예제 코드를 반영하게 되었다.
...
image의 경우, corner radius 적용이 필요했기 때문에 Glide 라이브러리를 사용해 이미지를 넣어주게 되었다. 

---

### #3 채팅 데이터 클래스 구현

→ 채팅 RecyclerView Item의 Data 클래스 생성 

❗서버에 넘겨줄 때 사용하는 데이터 클래스와, 레이아웃을 구성하기 위한 item의 데이터 클래스는 다를 수 있다
...
<aside>
💡 getTimeText()
> 이전에 ios 부스트 코스 강의를 수강하면서 배운 방식을 응용한 것이다.
데이터 클래스에서 처리할 수 있는 부분은 데이터 클래스내에서 처리하자는 생각
long 타입의 time 데이터를 activity 내에서 ‘HH:mm’ 형식으로 바꿔주는 것이 아닌, 데이터 클래스에서 바로 제공되도록 구현했다.
→ 채팅 activity에서 제공해야 할 부분 줄일 수 있음

</aside>

@ 참고

[https://velog.io/@dongchyeon/안드로이드-socket.io를-이용한-간단한-채팅-어플-만들어보기-3](https://velog.io/@dongchyeon/%EC%95%88%EB%93%9C%EB%A1%9C%EC%9D%B4%EB%93%9C-socket.io%EB%A5%BC-%EC%9D%B4%EC%9A%A9%ED%95%9C-%EA%B0%84%EB%8B%A8%ED%95%9C-%EC%B1%84%ED%8C%85-%EC%96%B4%ED%94%8C-%EB%A7%8C%EB%93%A4%EC%96%B4%EB%B3%B4%EA%B8%B0-3)